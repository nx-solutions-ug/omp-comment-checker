...
For `perFileResults`, `oldText` is looked up under `oldText`, `old_text`, `oldString`, or `old_string`. `newText` uses the `new*` variants. `filePath` accepts `filePath` or `file_path`; `movePath` accepts `movePath` or `move_path`. If `oldText` is empty, the request is treated as a `Write`. Entries with `success: false` are skipped; `success: true` is recorded but does not otherwise change the request.
