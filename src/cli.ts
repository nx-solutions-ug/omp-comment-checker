...
export async function runCommentChecker(
  ...
  if (!binaryPath) {
    return {
      status: 'missing',
      message:
        'comment-checker binary not found. Install @code-yeongyu/comment-checker or reload the package.',
    };
  }
...
export function spawnProcess(
  ...
  return new Promise((resolve) => {
    ...
    const timeoutLimit =
      Number.isFinite(processTimeoutMs) && processTimeoutMs > 0 ? Math.floor(processTimeoutMs) : 0;
    ...
    const timeoutTimer =
      timeoutLimit > 0
        ? setTimeout(() => {
            ...
            replaceOutput(
              stderr,
              `comment-checker process timed out after ${timeoutLimit} ms`,
              outputByteLimit,
            );
            proc.kill('SIGTERM');
